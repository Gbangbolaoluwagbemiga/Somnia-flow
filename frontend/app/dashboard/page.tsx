...
export default function DashboardPage() {
  ...
  // Subscribe to milestone updates using Somnia Data Streams
  useEffect(() => {
    if (
      !wallet.isConnected ||
      !subscribeToMilestoneUpdates ||
    ...
    const escrowIdsString = escrowIds.join(",");
    ...
    escrowIds.forEach((escrowId) => {
      ...
      subscribeToMilestoneUpdates(escrowId, (data) => {
        try {
          ...
          if (milestoneIndexField && statusField) {
          ...
          console.error("Error processing milestone update:", error);
        }
      })
        .then((unsubscribe) => {
  ...
  const disputeMilestone = async (escrowId: string, milestoneIndex: number) => {
    try {
      ...
      // Set a timeout to reload after 3 seconds regardless of receipt status
      const reloadTimeout = setTimeout(() => {
        console.log("Reload timeout triggered - reloading dashboard");
      ...
      // Clear the timeout since we're about to reload
  ...
  const approveMilestone = async (escrowId: string, milestoneIndex: number) => {
    try {
      ...
      // Set a timeout to reload after 3 seconds regardless of receipt status
      // This ensures reload happens even if receipt polling fails
      const reloadTimeout = setTimeout(() => {
        console.log("Reload timeout triggered - reloading dashboard");
        setSubmittingMilestone(null);
        window.location.reload();
      ...
      let receipt;
      let attempts = 0;
      const maxAttempts = 10; // Reduced attempts since we have timeout
